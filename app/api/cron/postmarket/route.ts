...
import { generatePostMarketEmail } from '@/lib/market-email-generator';
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const { subject, html } = await generatePostMarketEmail();
    ...
    console.log(`[postmarket-cron] Sent to ${sent} subscribers`);
    return NextResponse.json({ success: true, sent, subject });
  } catch (err) {
    console.error('[postmarket-cron] Error:', err);
